...
import {
  ...
  CheckCircle2, Phone, MapPin, Eye, EyeOff,
} from 'lucide-react';
...
/* ─────────────────────────────────────────────
   SOCIAL BUTTON
───────────────────────────────────────────── */
const SocialBtn: React.FC<{ icon: React.ReactNode; label: string; dark?: boolean }> = ({ icon, label, dark }) => (
  <button
    type="button"
    className="flex flex-1 items-center justify-center gap-2.5 rounded-xl py-3 text-sm font-semibold transition-all duration-200 hover:scale-[1.02] active:scale-[0.98]"
    style={{
      background: dark ? 'rgba(255,255,255,0.96)' : 'rgba(255,255,255,0.10)',
      border: dark ? '1px solid rgba(255,255,255,0.9)' : '1px solid rgba(255,255,255,0.16)',
...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, mode: initialMode, onClose }) => {
  ...
  useEffect(() => {
    if (isOpen) { setAnimIn(false); requestAnimationFrame(() => setTimeout(() => setAnimIn(true), 10)); }
  ...
  const switchMode = (m: 'signin' | 'login') => { setMode(m); setFormError(''); };

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setFormError('');
    if (mode === 'signin') {
      if (!name || !mobile || !location || !email || !password || !confirmPassword) {
      ...
      setFormError('Please enter your email and password.'); return;
    }
    setSuccess(true);
    setTimeout(() => { onClose(); setSuccess(false); setFormError(''); }, 2200);
  };
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            width: '70%', height: 1,
            background: 'linear-gradient(90deg, transparent, rgba(82,183,136,0.45), transparent)',
            pointerEvents: 'none',
          }}
        />
        ...
        <div className="overflow-y-auto flex-1 px-7 py-6">
          ...
          {success ? (
            <div className="flex flex-col items-center justify-center gap-4 py-12 text-center">
              ...
              <div>
                ...
                <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.48)', marginTop: 6 }}>
                  Redirecting to your eco dashboard…
                </p>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4" noValidate>
              ...
              >
                <span>{isSignIn ? 'Create free account' : 'Sign in to account'}</span>
